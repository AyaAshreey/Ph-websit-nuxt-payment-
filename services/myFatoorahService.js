// services/myFatoorahService.js
import axios from 'axios';

export const initiateSession = async () => {
    try {
        const response = await axios.post('https://apitest.myfatoorah.com/v2/InitiateSession', {}, {
            headers: {
        ...
        if (response.data.IsSuccess) {
            ...
            throw new Error('Failed to initiate session: ' + response.data.Message);
        }
    } catch (error) {
        console.error('Error initiating session:', error.response ? error.response.data : error.message);
        throw error;
    }
